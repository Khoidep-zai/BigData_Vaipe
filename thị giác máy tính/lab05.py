...
Original file is located at
    https://colab.research.google.com/drive/1BVDfoISskekYnBFA-Pvg4uGDoUaq6MSK
...
# This can be any directory you want to download FMNIST to
...
# download FMNIST to data_folder
...
"""Import các gói có liên quan và xác định thiết bị:"""
...
Nhìn chung, trong phần này, chúng ta sẽ tìm hiểu về tác động của các giá trị tốc độ học khác
nhau đối với các tập dữ liệu được chia tỷ lệ và không chia tỷ lệ.
Tác động của tốc độ học lên tập dữ liệu được chia tỷ lệ
Trong phần này, chúng ta sẽ so sánh độ chính xác của tập dữ liệu huấn luyện và xác thực với các
tốc độ học sau:
• Tốc độ học cao
...
def get_model():
    ...
    loss_fn = nn.CrossEntropyLoss()
    optimizer = Adam(model.parameters(), lr=1e-1)  # learning rate = 0.1
